...
### Hypothesis 2:
A machine learning model can predict whether a leaf is healthy or infected with powdery mildew with high accuracy.
...
**Rationale:** Develop a Machine Learning model that can automatically classify images of healthy or powdery mildew leaves. This involves:
...
- **Content:**
    - Lists the project hypotheses and how each was validated.
...
- **Content:**
    - Performance metrics of the ML model such as the learning curve, average accuracy, and loss in predictions.
...
- **Data Usage:** The data will only be used for the purposes of this project, and any outputs (such as visualizations or models) will not expose sensitive or identifying information. All training and test datasets won't be shared in the project repo.
...
- **Model Type:** A binary classification model will be used to predict whether a cherry leaf is healthy or infected. For this project, a Convolutional Neural Network will be used.
...
1. **Pandas**
   ...
   - **Usage Example:** You used Pandas to create a frequency DataFrame for counting images in different sets (train, validation, test) based on their labels.
     ```python
...
3. **Matplotlib**
   ...
   - **Usage Example:** You used Matplotlib to create scatter plots for visualizing resized image dimensions and bar plots for the distribution of images in different sets.
     ```python
...
4. **Seaborn**
   ...
   - **Usage Example:** You used Seaborn to enhance your plots, such as scatter plots and learning curves of the model’s performance.
     ```python
...
5. **TensorFlow/Keras**
   ...
   - **Usage Example:** You used Keras to build a convolutional neural network (CNN) for image classification, defining the model architecture and training it on the augmented dataset.
     ```python
     model = Sequential()  
...
8. **Streamlit**
   - **Purpose:** Streamlit was used to create the frontend dashboard for the project.
   - **Usage Example:** You used it to build the summary page of the dashboard.
     ```python
     ...
     def page_project_summary_body():  
         ...
         st.info(  
             ...
             f"In cherry leaves, it generates a coat preventing sunlight from reaching the plant, obstructing photosynthesis, "  
             f"compromising the tree's health, and reducing the quality of the cherries.\n\n"  
             f"It not only affects tree health, but can also cause respiratory irritation in people sensitive to it. "  
             f"Our client Farmy & Foods spends approximately 30 minutes per tree to check for infection. "  
             f"An extra minute is taken to kill the fungus if infection is found. This process takes a lot of time and resources.\n\n"  
...
- Based on the walkthrough from [Malaria Detector Project](https://github.com/Luisg882/Malaria-Detector) which helped in structuring the project coding sequences to meet business requirements.

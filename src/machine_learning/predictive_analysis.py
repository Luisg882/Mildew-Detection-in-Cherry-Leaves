...
def resize_input_image(img):
    ...
    """
    image_average_size = load_pkl_file(file_path=f"outputs/v1/image_average_size.pkl")
    img_resized = img.resize((image_average_size[1], image_average_size[0]), Image.LANCZOS)
...
def load_model_and_predict(my_image):
    ...
    model = load_model(f"outputs/v1/mildew_detection_model.h5")

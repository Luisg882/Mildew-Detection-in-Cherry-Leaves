...
st.info(
    ...
    f"healthy and powedery mildew cherry leaves"
)


if st.checkbox("Difference between average and variability image"):
    st.warning(
        f"The average variability image between healty and powedery mildew leaves "
        f"shows a small difference in the shape of the leaves being the healty ones "
        f"a small circular shape, in the other hand, the powedery mildew leaves show a oval shape.\n\n"
        f"It aslo shows a spread pigmentation in the mildew leaves around the center, "
        f"giving sings of the wither powder caractheristic of the mildew fungus."
        f"Nonetheless this difference is not enought to differentiate the between the to classes."
    )

    average_healty_leave = st.image('outputs/v1/avg_var_healthy.png', caption='Average Healthy Leaf', use_column_width=True)
    average_mildew_leave = st.image('outputs/v1/avg_var_powdery_mildew.png', caption='Average Powdery Mildew Leaf', use_column_width=True)


if st.checkbox("Differences between average healthy and powdery mildew leaves"):
    st.warning(
        f"With this patterns we can't visually demostrate the difference between classes."
    )

    average_healty_leave = st.image('outputs/v1/avg_diff.png', caption='Difference between average images', use_column_width=True)
